...
extension ViewController {
    ...
    private func randomImage() -> NSURL {
        let index = random() % sampleImages.count
        return NSURL(string: sampleImages[index])!
    }
    private func randomText(min min: Int, max: Int) -> String {
        // let sample = ViewController.sample
        let charCount = sample.characters.count
        let length = random() % (max - min) + min
        let beginOffset = random() % (charCount - length)
