...
public enum SQLiteDatabaseType: String {
    case TEMP = "TEMP"
...
public enum SQLiteConflict: String {
    case NONE = ""
...
public enum SQLiteSORT: String {
    case NONE = ""
    case ASC = "ASC"
    case DESC = "DESC"
}

public enum SQLiteCapacity {
    case None
...
public struct SQLiteColumn {
    ...
    public init(name: String, type: SQLiteDataType) {
        self.init(name: name, type: type, size: SQLiteCapacity.None, constraints: [])
    }
    public init(name: String, type: SQLiteDataType, size: SQLiteCapacity) {
        self.init(name: name, type: type, size: size, constraints: [])
    }

    public init(name: String, type: SQLiteDataType, constraints: [SQLiteConstraint]) {
        self.init(name: name, type: type, size: SQLiteCapacity.None, constraints: constraints)
    }

    public init(name: String, type: SQLiteDataType, size: SQLiteCapacity, constraints: [SQLiteConstraint]) {
        self.name = name
        self.type = type
        self.size = size
...
public enum SQLiteConstraint {
    ...
    var rawValue: String {
        switch self {
        case .PRIMARY_KEY:
            return "PRIMARY KEY \(SQLiteSORT.ASC.rawValue) \(SQLiteConflict.REPLACE.rawValue)"
        case .NOT_NULL:
            return "NOT NULL"
        case .UNIQUE:
            return "UNIQUE"
